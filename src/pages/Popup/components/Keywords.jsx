...
const Keywords = ({
  ...
  let date = moment(
    moment(
      moment(users?.current_plan?.updated_at).add(30, 'days')
    ).toISOString()
  )
    .format('DD-MM-YYYY')
    .toString();
  ...
  function addItem(keywordId) {
    console.log(keywordId);
    const doesStartWithUpwork = () => RSS.startsWith('https://www.upwork.com');
    const isURLRss =
      RSS.startsWith('https://www.upwork.com/ab/feed/topics/rss') ||
      RSS.startsWith('https://www.upwork.com/ab/feed/jobs/rss')
        ? true
        : false;
  ...
  function numberOFJobs(keyword) {
    if (keywords && keywords.length !== 0) {
      ...
      if (
        jobs &&
        jobs.length !== 0 &&
        jobs.some((job) => job.keyword === keyword)
      ) {
        ...
        localJobs &&
        localJobs.length !== 0 &&
        localJobs.some((job) => job.keyword === keyword)
      ) {
  ...
  function addItemWhileLoading(keywords) {
    console.log('Le', localStorageService.getItem('keywords'));
    if (window.localStorage.getItem('keywords') === null) {
      for (let i = 0; i < keywords.length; i++) {
        console.log('Da', keywords[i].keyword, keywords[i].rss);
        const doesStartWithUpwork = () =>
          keywords[i].rss.startsWith('https://www.upwork.com');
        const isURLRss =
          keywords[i].rss.startsWith(
            'https://www.upwork.com/ab/feed/topics/rss'
          ) ||
          keywords[i].rss.startsWith('https://www.upwork.com/ab/feed/jobs/rss')
            ? true
            : false;
        ...
        if (
          ...
          if (!keywords[i].keyword && !keywords[i].rss) {
            setKeywordError(true);
            setRssError(true);
            setErrorMessage('All fields must be filled');
          }
  ...
  const getKeyword = () => {
    getPostCall('keywords', 'get', '', users?.token)
      .then(async (e) => {
        // console.log(window.localStorage.getItem('keywords'));
        if (window.localStorage.getItem('keywords') === null) {
          console.log(e?.data?.data.length);
          addItemWhileLoading(e.data?.data);
        }
      })
      .catch((e) => {
  ...
  const addKeywords = () => {
    ...
    getPostCall('keywords', 'post', data, users?.token)
      .then((response) => {
        console.log('sssssssss', response.data);
        if (response?.data?.success) {
          addItem(response.data?.data?.id);
        } else {
  ...
  return (
    <div className="app">
      ...
      <div className="bg-[#000000] px-[32px] relative">
        <h1 className="py-[32px] font-medium uppercase text-[20px] ">
          KEYWORDS
        </h1>
        {keywords &&
          keywords.map((item) => (
            ...
            >
              <div className="flex gap-[16px]">
                ...
                <Link
                  state={[item, numberOFJobs(item.keyword)]}
                  to={`${item?.location}/${item?.keywordId}`}
                  className="flex flex-1 justify-between items-center"
                >
                  <div>
                    <span className="text-[#999999] block text-[14px] capitalize ">
                      keyword
                    </span>
                    <span className="text-[16px] block">{item.keyword}</span>
                  </div>
                  {numberOFJobs(item.keyword) > 0 && (
        ...
        <div className="flex gap-[8px]">
          <input
            ...
            id="keyword"
            className={`${
              keywordError ? 'border-red-500  ' : 'border-[#999999]'
            } outline-none   bg-[#282828] text-[16px]  px-[16px] flex-1 rounded-[4px] border  h-[53px]`}
          />
          <input
            ...
            id="keyword"
            className={`${
              RssError ? 'border-red-500  ' : 'border-[#999999]'
            } bg-[#282828] text-[16px] px-[16px] outline-none flex-1 rounded-[4px] border border-[#999999] h-[53px]`}
          />
        </div>
        <span className="text-red-500 pt-[15px] block text-[14px]">
          {errorMessage}
        </span>
        <button
          onClick={() => addKeywords()}
